...
export async function sendPasswordResetEmail(to: string, resetUrl: string): Promise<void> {
  ...
  await transporter().sendMail({
    from: `NFL Pick'em <${from}>`,
    to,
    subject: "Reset your NFL Pick'em password",
    text: [
      "Someone requested a password reset for your NFL Pick'em account.",
      '',
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; line-height: 1.5; color: #14241c;">
        ...
        <p>Someone requested a password reset for your NFL Pick'em account.</p>
        <p>
          <a href="${resetUrl}"
             style="display:inline-block; background:#e23b2e; color:#fff; padding:10px 18px;
